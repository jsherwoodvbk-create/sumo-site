...
async function main() {
  ...
  // ── fold in the static historical layer (sumo-history.json, committed by the backfill) ──
  // Past basho only (Jan 2025 onward); NEVER gated. Powers query_career / query_yusho / historical H2H.
  let history = null;
  try {
  ...
  // ── fold in the next-day card (tomorrow-card.json, written by gen-torikumi.mjs) ──
  // UNGATED: an upcoming matchup has no result, so it is never a spoiler. gateSnapshot passes it
  // through untouched; query_upcoming reads it. Regenerated each run; absent/empty is fine.
  let upcoming = null;
  try {
    ...
    console.log(upcoming.empty
      ? '  + upcoming: none (empty card — not posted yet or basho over)'
      : `  + upcoming: Day ${upcoming.day} card, ${upcoming.matchups.length} matchups (ungated)`);
  ...
  const snapshot = {
    meta: {
      ...
      maxDay, schema: 'gumbai-snapshot/3', source: 'notion',
    },
