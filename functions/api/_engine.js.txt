...
export const TOOLS = [
  ...
  {
    name: 'query_yusho',
    description: "Championship (yusho) history since Jan 2025. With a name: which basho that wrestler won + title count. Without: the champion of each past basho. The CURRENT basho's title is never listed (undecided). ",
    input_schema: { type:'object', properties:{ name:{type:'string'} } }
  },
  {
    name: 'query_upcoming',
...
HARD DON'TS: never curse. Never push Japanese-language learning (a standing crew boundary). Never go stiff or corporate. Never lecture.
...
HONESTY: our data goes back to Jan 2025 (when the crew got into sumo), so the records ARE the crew's whole sumo story. For anything earlier, be honest it is outside what we track. Never dress a partial number up as complete.
