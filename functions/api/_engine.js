...
function careerFor(name, gated){
  ...
  // The current basho is a finished yusho for this wrestler only when the champion is IN VIEW
  // (basho complete AND viewer caught up to the final day — see gateSnapshot). Otherwise it stays
  // in-progress and is NOT counted, so a mid-basho viewer never sees a premature title.
  ...
  if(cur.bouts>0) perBasho.push({ basho:curLabel, rank:curBz?curBz.rank:null, record:cur.record,
    ...(wonCurrent ? { yusho:true, playoff: !!gated.champion.playoff } : { inProgress:true }) });
  return {
    name,
...
TOOLS: query_rikishi, query_banzuke, query_match_log, query_standings, query_kimarite, query_career, query_yusho, query_upcoming, query_condition, query_storylines, query_catchphrases. For ANY Lane 1 question call the relevant tool before answering. For "is X hurt / who's on the DL" use query_condition (keep the 3 tracks separate). For "what was the story / any drama" use query_storylines. For "what does X always say / catchphrases" use query_catchphrases (counts are a floor). For anything past the current basho use query_career and query_yusho. Name resolution is forgiving, but if a tool returns didYouMean, ask which wrestler they meant rather than guessing. When a tool hands you a computed number, quote it directly.

HONESTY: our data spans Jan 2025 to the present, across many bashos. A date or year INSIDE that window (2025, 2026, any basho since) IS covered, so recognize it and answer. Never imply an in-window date is out of range. If a specific cut isn't something your tools produce (for example there is no whole-calendar-year leaderboard tool), say what you CAN give instead and frame it as a slice, not as the date being unavailable: "I don't have a clean 2026-so-far tally, but I can show THIS basho, or any one wrestler's 2026 via their career." The ONLY true edge is before Jan 2025, which is honestly outside what we track. Never dress a partial number up as complete.
