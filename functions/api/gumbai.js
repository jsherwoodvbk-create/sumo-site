...
import { gateSnapshot, buildSystemPrompt, TOOLS, runTool, ENGINE_VERSION } from './_engine.js';
...
export async function onRequestGet(ctx){
  ...
  return json({
    ...
    tools: TOOLS.map(t=>t.name),                  // should include query_leaderboard once the new engine is deployed
    snapshotHasChampion: !!SNAP.champion,         // true once the snapshot regenerated on a completed basho (no name = no spoiler)
    loggingConfigured: !!(env.GUMBAI_LOG_DB && (env.GUMBAI_LOG_TOKEN || env.NOTION_TOKEN)), // interaction-log env wired?
  });
}
