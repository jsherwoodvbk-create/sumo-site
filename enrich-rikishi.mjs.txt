// enrich-rikishi.mjs (v7) — full rikishi enrichment PROBE + writer, sourced from sumo-api.
...
// NOTE: requires "Past Stables", "Debut" (date), "Retirement" (date) on Master Rikishi; plus a "Past Names" (text) field on Stables.
...
async function main() {
  console.log(`enrich-rikishi v7: DRY_RUN=${DRY ? 'ON (no writes)' : 'OFF (WRITING)'}\n`);
  ...
  for (const p of stableRows) {
    const n = textOf(p, 'Name');
    if (n) { stableMap.set(normHeya(n), p.id); idToName.set(p.id, n); }
    // Alias a stable's former names to its CURRENT page, so sumo-api's lagging/old heya (e.g. a
    // renamed stable) still resolves correctly and never fires a false wrestler-level stable change.
    for (const alias of textOf(p, 'Past Names').split(',').map(s => s.trim()).filter(Boolean)) stableMap.set(normHeya(alias), p.id);
  }
